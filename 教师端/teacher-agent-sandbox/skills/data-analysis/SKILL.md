...
- **File location**: Path(s) to uploaded Excel/CSV files under `（上传文件目录）/`
...
- Use the actual uploaded file paths provided by the user.
...
python skills/data-analysis/scripts/analyze.py \
  ...
  --output-file （输出目录）/filtered-results.csv
```
...
python skills/data-analysis/scripts/analyze.py \
  ...
  --output-file （输出目录）/monthly-trends.csv
```
...
```bash
python skills/data-analysis/scripts/analyze.py \
  --files （上传文件目录）/orders.csv （上传文件目录）/customers.xlsx \
  --action query \
...
- On first load, files are parsed and stored in a persistent DuckDB database under a local cache directory
